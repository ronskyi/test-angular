...
export class AnimalFormStore extends ComponentStore<AnimalFormState> {
  ...
  readonly updateSpeciesWithKeyword = this.effect(
  ...
  readonly updateOwnersWithKeyword = this.effect(
    (data$: Observable<string>) =>
      data$.pipe(
        debounceTime(500),
        filter((v ) => v.length > 2),
  ...
  private readonly fetchOwners = this.effect(
    (data$: Observable<string>) =>
      data$.pipe(
  ...
  private readonly fetchSpecies = this.effect(
    (data$: Observable<string>) =>
      data$.pipe(
  ...
  private readonly setSpecies = this.updater(
    (state, items: Specie[]) => {
  ...
  private readonly setOwners = this.updater(
    (state, items: Owner[]) => {
