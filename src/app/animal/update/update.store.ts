...
export class UpdateAnimalStore extends ComponentStore<UpdateState<Animal>> {
  ...
  readonly fetchItem = this.effect((data$: Observable<string>) =>
    data$.pipe(
      tap(() => this.changeState(UpdateStateEnum.LOADING)),
      switchMap((id) =>
        this.animalService.fetchById(id).pipe(
          catchError((err) => {
            console.log(err);
  ...
  readonly update = this.effect((data$: Observable<Animal>) =>
    data$.pipe(
      tap(() => this.changeState(UpdateStateEnum.WAITING_RESPONSE)),
      withLatestFrom(this.item$),
      switchMap(([data, item]) =>
        this.animalService.update({ ...data, id: item!.id }).pipe(
          catchError((err) => {
            console.log(err);
            this.changeState(UpdateStateEnum.UNKNOWN_ERROR);
            return EMPTY;
  ...
  private readonly setItem = this.updater((state, item: Animal) => {
