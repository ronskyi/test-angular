...
export class UpdateSpecieStore extends ComponentStore<SpecieCreateState> {
  ...
  readonly fetchOwner = this.effect((data$: Observable<string>) =>
    data$.pipe(
      ...
      switchMap((data) =>
        this.specieService.fetchById(data).pipe(
          catchError((err) => {
  ...
  readonly update = this.effect((data$: Observable<Specie>) =>
    data$.pipe(
      tap(() => this.changeState(SpecieUpdateStateEnum.WAITING_RESPONSE)),
      withLatestFrom(this.item$),
      switchMap(([data, specie]) =>
        this.specieService.update({ ...data, id: specie!.id }).pipe(
          catchError((err) => {
            this.changeState(SpecieUpdateStateEnum.UNKNOWN_ERROR);
            return EMPTY;
          }),
  ...
  private readonly setSpecie = this.updater((state, item: Specie) => {
