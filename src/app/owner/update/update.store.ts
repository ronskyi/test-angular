...
export class UpdateOwnerStore extends ComponentStore<OwnerCreateState> {
  ...
  readonly fetchOwner = this.effect(
    (data$: Observable<string>) =>
      data$.pipe(
        ...
        switchMap((data) => this.ownerService.fetchById(data).pipe(
          catchError(err => {
  ...
  readonly update = this.effect(
    (data$: Observable<Owner>) =>
      data$.pipe(
        ...
        switchMap(([data, owner]) => this.ownerService.update({...data, id: owner!.id}).pipe(
          catchError(err => {
            this.changeState(OwnerUpdateStateEnum.UNKNOWN_ERROR)
            return EMPTY;
          })
  ...
  private readonly setOwner = this.updater(
    (state, owner: Owner) => {
